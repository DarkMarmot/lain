...
## Installation
Install the module with: `npm install lain` or place into your `package.json`
and run `npm install`.
...
* `multiWrite(writes, [dimension])` Finds and writes to multiple `Data` instances at once, notifying subscribers only after all writes have
 been completed. The `writes` argument can be either a hash of names and values or an array of `{name: n, value: v, topic: t}` objects.
* `kills(destructible, [destructor])` Ties the lifecycle of a destructible object to the scope. When the scope is destroyed or cleared,
...
The `Data` class is a generic pub/sub box for holding data values by topic. If no topic is specified, the values are stored with a `null` topic.
